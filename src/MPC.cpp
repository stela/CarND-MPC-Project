...
// Offsets into the variables array used for solving (state and actuators across timesteps)
...
class FG_eval {
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    // Minimize cte and epsi. Minimize deviation from reference velocity
    for (int i = 0; i < N; i++) {
    ...
    // prefer reduced usage of actuators (steer + acceleration) if possible, especially if combined
    for (int i = 0; i < N - 1; i++) {
    ...
    // prefer to steer and accelerate similar to previous step, for smoother actuator control
    // and less car-sick passengers ;-)
    for (int i = 0; i < N - 2; i++) {
      fg[0] += 150 * CppAD::pow(vars[delta_start + i + 1] - vars[delta_start + i], 2);
      fg[0] += 7 * CppAD::pow(vars[a_start + i + 1] - vars[a_start + i], 2);
    }


    // remaining fg elements contain the vars items (offset 1 compared to vars, since cost at [0])
...
vector<double> MPC::Solve(const Eigen::VectorXd &state, const Eigen::VectorXd &coeffs) {
  ...
  // 4 * 10 + 2 * 9
  //
  // we have 6 state variables, 2 actuators, N=10 state-timesteps, and N-1 actuator-timesteps
  const size_t n_vars = 6 * N + 2 * (N - 1);
  // 6 constraints below, non-actuators finite, steering angle delta [-25 degrees tp +25 degrees,
  // and accelerator [-1, +1]
